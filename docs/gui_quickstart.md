...
| p_place | 予測複勝圏確率 (4桁) |
| horse_edge (p-avg) | 馬の p_place - レース内平均 p_place (3桁)。プラスほど相対的に有力 |
| race_confidence (p1-p2) | レース内 1位と2位の p_place の差 (3桁)。値が大きいほど1頭突出で信頼度が高い |
...
| **最小p_place ≥** チェックボックス + スピンボックス | チェックで有効化。p_place がこの値以上の馬のみ表示 (戦略A: 高確信馬のみ購入) |
| **最小confidence ≥** チェックボックス + スピンボックス | チェックで有効化。race_confidence (p1-p2) がこの値以上のレースのみ表示 (信頼度の高いレースのみ対象) |

フィルタはリアルタイムで適用されます (ファイルの再読み込みは不要)。各フィルタの値とチェック状態はアプリ終了時に自動保存され、次回起動時に復元されます。
