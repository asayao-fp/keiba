...
| 騎手 | `jockeys` テーブルがある場合はドロップダウン。ない場合は騎手コードを直接入力 |
| 調教師 | `trainers` テーブルがある場合はドロップダウン。ない場合は調教師コードを直接入力 |
| 斤量 (kg) | 数値 (例: `57.5`) |
| **馬体重 (kg)** | **必須**。整数 (例: `480`) |
...
1. **「マスタ読み込み」** ボタンを押して DB から馬・騎手・調教師マスタを読み込みます。
   - `horses` / `jockeys` / `trainers` テーブルがない場合は自動でフリーテキスト入力に切り替わります。
...
- 各出走行の馬番・馬名/horse_id・騎手・調教師・斤量・**馬体重** はすべて必須です。
...
| `jockeys` | ✓ | 騎手名ドロップダウンを表示 |
| `jockeys` | ✗ | 騎手コードを直接入力するテキストフィールドを表示 |
| `trainers` | ✓ | 調教師名ドロップダウンを表示 |
| `trainers` | ✗ | 調教師コードを直接入力するテキストフィールドを表示 |
