...
| 距離 (m) | **「プリセット選択」ドロップダウン**でよく使われる距離 (1000m〜3600m) を選択するとスピンボックスに自動反映されます。プリセットにない距離はスピンボックスに直接入力してください |
| 馬場状態 | 良 / 稍重 / 重 / 不良 から選択 |
| 馬場種別 | 芝 / ダート から選択。**学習済みモデルが馬場種別特徴量を含む場合は必須**。モデルが対応していない場合はラベルに "(任意)" と表示され、予測に影響しません |
...
- **馬場種別** はモデルの特徴量に含まれる場合のみ必須です。ラベルに `*` が表示されている場合は選択必須です。含まれない場合は選択不要で予測に影響しません。
