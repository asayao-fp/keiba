...
| 馬場状態 | 良 / 稍重 / 重 / 不良 から選択 |
| 馬場種別 (芝/ダート) | 芝 / ダート から選択。モデルが馬場種別特徴量に対応している場合は必須、対応していない場合は任意 (予測に影響しません) |
...
- **馬場種別 (芝/ダート)** は、現在ロードされているモデルの特徴量列に馬場種別列 (`surface`, `surface_code`, `track_surface` のいずれか) が含まれる場合のみ必須です。モデルが馬場種別に対応していない場合は省略可能で、入力値は予測に使用されません。
